...
namespace Surfer
{
    ...
    public partial class SUElement
    {
        ...
        public System.Action<float> OnDragStateProgress;
        ...
        void CheckDragTypeForMyStateEnter()
        {
            ...
            transform.DOMove(_dragPosIn, _dragSpeed).OnUpdate(() =>
            {
        ...
        void CheckDragTypeForMyStateExit()
        {
            ...
            transform.DOMove(_dragPosOut, _dragSpeed).OnUpdate(() =>
            {
                SendDragProgressEvent();
            }).Play();
        ...
        void SendDragProgressEvent()
        {
            float progress = 0;

            if (_elementData.IsHorizontalDrag())
            {
                progress = (transform.position.x - _dragPosIn.x) / (_dragPosOut.x - _dragPosIn.x);
            }
            else
            {
                progress = (transform.position.y - _dragPosIn.y) / (_dragPosOut.y - _dragPosIn.y);
            }

            progress = (1 - progress);
            OnDragStateProgress?.Invoke(progress);
