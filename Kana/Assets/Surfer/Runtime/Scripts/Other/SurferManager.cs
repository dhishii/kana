...
#region Public
        ...
        public const string kStateEnterVisualScripting = "_SUStateEnter";
        public const string kStateExitVisualScripting = "_SUStateExit";
        public const string kSceneLoadedVisualScripting = "_SUSceneLoaded";
        public const string kSceneUnloadedVisualScripting = "_SUSceneUnloaded";
        ...
        void Awake()
        {
            ...
            if(I==null)
            {
                ...
                gameObject.AddComponent<SUInputManager>();
                gameObject.AddComponent<SUThemeManager>();
        ...
        private void Update()
        {
            ...
            SUInputManager.I?.MainLoop();
            SUSafeAreaManager.I?.MainLoop();
        }
        ...
        void OnSceneLoaded(Scene scene , LoadSceneMode mode)
        {
            ...
            var eventData = new SUSceneLoadedEventData(scene.name);
            ...
            if(_sceneLoadedReg.TryGetValue(scene.name,out HashSet<ISUSceneLoadedHandler> set))
            {
                ...
                foreach (ISUSceneLoadedHandler item in set)
                {
                    ...
                    item.OnSUSceneLoaded(eventData);
                }

            }

#if SUVis
            EventBus.Trigger(kSceneLoadedVisualScripting,eventData);
#endif
        ...
        void OnSceneUnloaded(Scene scene)
        {
            ...
            var eventData = new SUSceneUnloadedEventData(scene.name);
            ...
            if(_sceneUnloadedReg.TryGetValue(scene.name,out HashSet<ISUSceneUnloadedHandler> set))
            {
                ...
                foreach(ISUSceneUnloadedHandler item in set)
                {
                    ...
                    item.OnSUSceneUnloaded(eventData);
                }
            }

#if SUVis
            EventBus.Trigger(kSceneUnloadedVisualScripting,eventData);
#endif
        ...
        public void OpenPrefabState(GameObject prefab,Transform parent = null, int version = SurferHelper.kWhateverVersion,float delay = _noDelay, params object[] customData)
        {
            ...
            {
                stateName = prefab.GetComponent<SUElement>().ElementData.StateName;
...
#if SUVis
            EventBus.Trigger(kStateEnterVisualScripting,eventData);
#endif
        }
...
#if SUVis
            EventBus.Trigger(kStateExitVisualScripting,eventData);
#endif

            //state stack 

            var eleData = GetStateElementData(state,playerID);

            if(eleData == null)
                return;
            if(!eleData.IsStackable)
                return;
            
            if(!eleData.IsStackDelayRunning)
            {
                _stackRoutine = StartCoroutine(StackRoutine(eleData));
            }
...
#region  Other
        ...
        void CheckParentNesting(string state,int playerID)
        {
            ...
            for (int i = 0; i < eleData.ParentUIStates.Count; i++)
            {
                OpenPlayerState(eleData.ParentUIStates[i].PlayerID,eleData.ParentUIStates[i].Name);
            }
        ...
        void CheckCloseMode(string state, int playerID)
        {
            ...
            List<SUStateInfo> toClose = eleData.StatesToClose;

            for (int i = 0; i < toClose?.Count; i++)
            {
                ClosePlayerState(toClose[i].PlayerID, toClose[i].Name, eleData.CloseDelay,null);
            }
        ...
        bool IsStackDelayRunning(string state, int playerID )
        {
            ...
            return eleData.IsStackDelayRunning;
        }
        ...
        void CheckChildNesting(string state, int playerID)
        {
            ...
            for (int i = 0; i < eleData.ChildUIStates.Count; i++)
            {
                ClosePlayerState(eleData.ChildUIStates[i].PlayerID,eleData.ChildUIStates[i].Name);
            }
        ...
        /// </summary>
        public SUElementData GetStateElementData(string stateName, int playerID)
        {
            if (string.IsNullOrEmpty(stateName))
                return null;
            ...
            {
                if(!item.Type.IsState())
                    continue;
                if (item.StateName.Equals(stateName)
        ...
        IEnumerator StackRoutine(SUElementData eleData)
        {

            eleData.IsStackDelayRunning = true;

            yield return new WaitForSeconds(eleData.StackDelay);

            eleData.IsStackDelayRunning = false;
            SurferManager.I.CheckStateStack(eleData.StateName,eleData.PlayerID);
