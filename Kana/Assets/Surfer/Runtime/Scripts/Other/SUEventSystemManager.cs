...
namespace Surfer
{
    ...
    public partial class SUEventSystemManager : MonoBehaviour
    {
            ...
            for(int i=0;i<_allEventSystems.Count;i++)
            {
                ...
                if (_loopItem.currentSelectedGameObject != _objectSelected)
                {
                    ...
                    if (_loopItem.currentSelectedGameObject == null)
                        ...
                        _newFocusState = _loopItem.currentSelectedGameObject.GetObjectStateName(true);
        ...
        void Follow(int playerID, bool isNewTarget)
        {
            ...
            if (toFollow == null)
            {
                if (indicator.transform.position.y > SurferHelper.OutPos.y)
                    indicator.transform.position = SurferHelper.OutPos;
                return;
            }
        ...
        public void FocusOnObject(GameObject obj)
        {
            ...
            var eleData = obj.GetParentState();

            if (eleData == null)
                return;

            //send log!
            if (_allEventSystems[eleData.PlayerID] == null)
                return;
            if (_allEventSystems[eleData.PlayerID].gameObject == null)
                return;

            _allEventSystems[eleData.PlayerID].SetSelectedGameObject(null);
            _allEventSystems[eleData.PlayerID].SetSelectedGameObject(obj);
        ...
        public void FocusOnObjectOrLast(GameObject obj)
        {
            ...
            var eleData = obj.GetObjectStateElementData();

            if (string.IsNullOrEmpty(eleData.StateName))
            {
                FocusOnObject(obj);
                return;
            }

            if (_statesSelections.TryGetValue(eleData.StateName, out Dictionary<int,GameObject> value)
                && value.TryGetValue(eleData.PlayerID, out GameObject selection))
            {
        ...
        public void CheckHistoryFocus(int playerID, string stateOpened)
        {
            ...
            if(_selectedObjects.TryGetValue(playerID, out GameObject selected)
                && stateOpened != selected.GetObjectStateName() )
            {
