...
namespace Surfer
{
    ...
    {           
        ...
        [SerializeField]
        [HideInInspector]
        float _doubleClickDelay = 0.15f;
        public float DoubleClickDelay { get => _doubleClickDelay; set => _doubleClickDelay = value; }


        [SerializeField]
        ThemesDictionary _themes = new ThemesDictionary();
        public ThemesDictionary Themes { get => _themes; }

        [SerializeField]
        string _selectedThemeGUID = default;
        public string SelectedThemeGUID { get => _selectedThemeGUID; set => _selectedThemeGUID = value; }

        [SerializeField]
        SUInputIconsDict _inputIcons = new SUInputIconsDict();
        public SUInputIconsDict InputIcons { get => _inputIcons; }
        ...
        /// </summary>
        /// <param name="idx">theme key</param>
        /// <param name="newName">theme new name</param>
        public void RenameTheme(string key,string newName,System.Action OnFail,System.Action OnSuccess)
        {
            ...
            if(_themes.TryGetValue(key,out SUThemeData data))
            {
                _themes[key].Name = newName;
                OnSuccess?.Invoke();
            }
        ...
        /// Add a new theme. 
        /// </summary>
        /// <param name="newName">new theme name</param>
        public void AddTheme(string newName,System.Action OnFail,System.Action<string> OnSuccess)
        {
            if(string.IsNullOrEmpty(newName))
            return;
            ...
            _themes.Add(newGUID,new SUThemeData(newName,_themes.FirstOrDefault().Value));
            OnSuccess?.Invoke(newGUID);
        ...
        /// </summary>
        /// <param name="newName">new theme name</param>
        public void AddInputIconsPlatform(SUPlatform_ID platform,System.Action OnFail,System.Action OnSuccess)
        {

            if (_inputIcons.ContainsKey(platform))
            {
                OnFail?.Invoke();
                return;
            }

            _inputIcons.Add(platform,new SUInputIconsData(_inputIcons.FirstOrDefault().Value));
            OnSuccess?.Invoke();
        ...
        /// <returns>key of the theme</returns>
        public string GetThemeKey(string name)
        {
            foreach(KeyValuePair<string,SUThemeData> pair in _themes)
            {
        ...
        {
            if(_inputIcons.TryGetValue(platformID,out SUInputIconsData data))
            return data;
        ...
        {
            return _inputIcons.Select(x=>x.Key.ToString()).Where(x=>!string.IsNullOrEmpty(x)).OrderBy(x=>x).Prepend(SurferHelper.Unset).ToArray();
        }
    }
