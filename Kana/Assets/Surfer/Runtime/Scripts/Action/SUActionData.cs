...
#region Serialized Fields
        ...
        {
            if(!_conds.AreAllSatisfied(new SUElementData() { ObjOwner = go },null))
            return;

            if(_playerID == SurferHelper.kNestedPlayerID)
            {
                _playerID = go.GetObjectStatePlayerID();
            }
            ...
            else if(_mode == SUAction_ID.OpenPrefabState)
            {
                if(_parentMode == SUPrefabParent_ID.Scene)
                    ...
                    SurferManager.I.OpenPrefabState(_prefab, go.GetObjectStateTransfom(), _version, _delay);
                else if (_parentMode == SUPrefabParent_ID.ThisStateParent)
                    SurferManager.I.OpenPrefabState(_prefab, go.GetObjectStateTransfom().parent.gameObject.GetObjectStateTransfom(), _version, _delay);
            ...
            {
                SurferManager.I.ClosePlayerState(go.GetObjectStatePlayerID(true), go.GetObjectStateName(true), 0, _delay);
            }
